...
func (r *Repository) Create(ctx context.Context, params CreateRepoParams) (domain.Course, error) {
	const query = `
		insert into course (title, description, owner_user_id)
		values ($1, $2, $3)
		returning id, title, description, owner_user_id, status, created_at, updated_at`

	var course courseModel
	err := r.db.GetContext(ctx, &course, query, params.Title, params.Description, params.OwnerUserId)
	if err != nil {
...
func (r *Repository) Update(ctx context.Context, params UpdateRepoParams) (domain.Course, error) {
	...
	err := r.db.GetContext(ctx, &course, query,
		params.ID, params.Title, params.Description, params.OwnerUserId, params.Status)
	if err != nil {
...
func (r *Repository) Get(ctx context.Context, id int64) (domain.Course, error) {
	const (
		getCourseQuery = `
			select id, title, description, owner_user_id, status, created_at, updated_at
			from course
			where id = $1`
		countListenersQuery = `
...
func (r *Repository) GetList(ctx context.Context, params GetListRepoParams) ([]domain.Course, error) {
	const (
		listCoursesBaseQuery = `
			select distinct c.id, c.title, c.description, c.owner_user_id, c.status, c.created_at, c.updated_at
			from course c`
	...
	for _, m := range models {
		courses = append(courses, m.toDomain(domain.CourseStats{}))
	}
	return courses, nil
}
